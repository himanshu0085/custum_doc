...
**Refs inspected:** `origin/qa`, `origin/uat`, `origin/prod`
...
| Standard service pipelines | `deploy/azurepipeline/pipeline-actions.yaml` through `pipeline-users.yaml` | Per-service build/push/deploy | QA, UAT, PROD variants | Repeated service implementations; no common template references |
| Frontend pipeline | `deploy/azurepipeline/pipeline-frontend.yaml` | Frontend build/push/deploy | QA, UAT, PROD variants | Environment-specific variants differ materially |
...
| Secret injection | `inject-secrets.yaml`, UAT `prod/inject-secrets.yaml` | Create/update Kubernetes secrets and restart workloads | QA/UAT/PROD variants | Multiple secret mechanisms; active registration UNKNOWN |
...
| Rollback | `rollback-prod.yml` | Production rollback/inspection | QA/UAT/PROD refs | Branch naming is inconsistent; active registration UNKNOWN |
...
| Private ingress | `k8s/ingress-private.yaml` | Private/Application Gateway ingress | UAT/PROD refs | QA has a separate ingress pipeline; activation UNKNOWN |
...
| Network policy | `k8s/backend/network-policy.yaml` | Pod network restrictions | QA/UAT/PROD refs | Security control drift |
| Docker | `deploy/docker/backend/dockerfile`, frontend counterpart | Intended container entry points | QA frontend differs; backend/other placeholders vary | Not consistently active in service flow |
...
**CONFIRMED from top-level `origin/uat`:** most service pipelines trigger from `uat`, checkout BE/FE `uat`, use UAT-named ACR/resource group/AKS values, build Java 21 Maven modules, skip tests, generally have Checkmarx disabled, push Build ID and `latest`, apply one manifest using `az aks command invoke`, and restart/wait for rollout.
...
| Family | QA (`origin/qa`) | UAT (`origin/uat`) | PROD (`origin/prod`) | Material difference |
|---|---|---|---|---|
| Service triggers | `qa` path filters | Mostly `uat` | Often triggerless/manual-looking; registration UNKNOWN | Branch and trigger policy differ |
...
| Approval | Some manual validation evidence in QA ref; registration UNKNOWN | No common service approval visible | Explicit production ManualValidation in variants | PROD has stronger human gate, but is duplicated |
...
| Network policy | Present in ref | Present in UAT ref | Present in PROD ref | Activation/order UNKNOWN |
...
| `origin/qa` vs `origin/uat` | QA/UAT | Different service connections, ACR, AKS, refs, diagnostics | Fixes and controls diverge | Compare through a generated environment contract, not copied files |
...
| `k8s/backend/network-policy.yaml` | QA/UAT/PROD | Present in all three requested refs | Security posture and rules may still differ | Make policy part of a tested release contract |
...
| NetworkPolicy | Present in QA ref | Present in UAT ref | Present in PROD ref | Activation/order UNKNOWN |
...
|---|---|---|---|---|
| SEC-01 | Critical | `origin/uat:k8s/backend/configmap-aks.yaml` has `REDIS_PASSWORD` and `JWT_SECRET` in ConfigMap data | Credential theft or JWT forgery; repository history may retain values | Treat as exposed, rotate through incident/change process, remove secret material from ConfigMaps, use approved secret store |
...
**CONFIRMED across the three requested refs:** service manifests use namespace `adp`; many Deployments use one replica, `imagePullPolicy: Always`, and `latest`; readiness is frequently TCP-based with long startup delays; liveness is inconsistent; resource requests/limits are repeated but not policy-enforced in repository code.

**QA:** `origin/qa` adds `network-policy.yaml`, `new-secret.yaml`, disabled/public ingress variants, and certificate files. This may represent QA/DEV ambiguity.  
...
| Approval | Some QA-ref ManualValidation evidence; registration UNKNOWN | No common service approval visible | Explicit production ManualValidation in variants | Environment governance differs |
...
| F-03 | Secrets in ConfigMap | Security | Critical | `origin/uat:k8s/backend/configmap-aks.yaml` | Credential/JWT exposure | Rotate and migrate to approved secret delivery | Medium |
